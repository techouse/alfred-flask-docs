...
void main(List<String> arguments) {
  wrapMain(() async {
    try {
      ...
      final AlfredUserConfigurationSelect? flaskVersion =
          userDefaults?['flask_version'] as AlfredUserConfigurationSelect?;

      if (flaskVersion == null) {
        throw Exception('flask_version not set!');
      }

      List<String> query =
          args['query'].replaceAll(RegExp(r'\s+'), ' ').trim().split(' ');
      String? version = flaskVersion.config.value;
